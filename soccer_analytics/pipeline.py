...
def analyze(video: str, cfg: PipelineConfig, view: ViewTransformer,
    ...
    for fidx, rec in enumerate(records):
        ...
        for (tid, xyxy, cls, color) in rec.players:
            is_ref = cls == REFEREE
            ...
            if is_ref or not team_ok:
                ...
                t = team.assign_from_color(tid, color)
            if not is_ref:
                pos = kf.position
                players_xy[tid] = pos
                teams[tid] = t
    ...
    if metric:
        ...
        injury_model = HeuristicInjuryModel()
        feats = load.all_features()
        players_out, injury_out = {}, {}
        for tid, f in feats.items():
            ...
            r = injury_model.predict(f)
            injury_out[str(tid)] = {"risk": r.risk, "level": r.level, "factors": r.factors}
        stats["players"] = players_out
        stats["injury_risk"] = injury_out
        stats["shots_xg"] = shots.summary()
        stats["substitution_watch"] = [
            a.as_dict() for a in subs.advise(
                {int(t): j["risk"] for t, j in injury_out.items()})[:5]]
        r1 = tactic.report(1, possession.get(1, 0.0))
        r2 = tactic.report(2, possession.get(2, 0.0))
        stats["tactics"] = {"team1": r1.as_dict(), "team2": r2.as_dict()}
    else:
        ...
        stats["note"] = "pixel mode (no calibration): distances/speeds in pixels; " \
                        "load, metabolic power and injury need a pitch calibration."
